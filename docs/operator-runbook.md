...
**Cause:** A metadata-only (`--identity meta`) entry's source object was
...
3. If the source object cannot be restored, the entry is irrecoverable.
   Remove it with `fluxel rm -m "..." <path>`.
...
| Task | Command |
|------|---------|
| List locks | `fluxel lock list --repo <URI>` |
| Cleanup stale locks | `fluxel lock cleanup --repo <URI>` |
| Force-release a lock | `fluxel lock cleanup --repo <URI> --force <branch>` |
| Verify integrity | `fluxel verify --repo <URI>` |
| Check lock timeout | `fluxel lock list --repo <URI> --json` (see `expires_at`) |

The default lock timeout is **30 seconds** and is configured via
`lock_timeout_seconds` when constructing an `S3RepositoryStore`.
