...
MINISTACK_IMAGE="${DATAREF_MINISTACK_IMAGE:-nahuelnucera/ministack}"
MINISTACK_HOST="${DATAREF_MINISTACK_HOST:-127.0.0.1}"
MINISTACK_PORT="${DATAREF_MINISTACK_PORT:-4566}"
DEFAULT_ENDPOINT="http://${MINISTACK_HOST}:${MINISTACK_PORT}"
MANAGE_CONTAINER="${DATAREF_MINISTACK_MANAGE_CONTAINER:-auto}"

export DATAREF_MINISTACK_ENDPOINT="${DATAREF_MINISTACK_ENDPOINT:-$DEFAULT_ENDPOINT}"
export DATAREF_MINISTACK_ACCESS_KEY="${DATAREF_MINISTACK_ACCESS_KEY:-test}"
export DATAREF_MINISTACK_SECRET_KEY="${DATAREF_MINISTACK_SECRET_KEY:-test}"
export DATAREF_MINISTACK_REGION="${DATAREF_MINISTACK_REGION:-us-east-1}"

HEALTH_URL="${DATAREF_MINISTACK_ENDPOINT%/}/_ministack/health"
RESET_URL="${DATAREF_MINISTACK_ENDPOINT%/}/_ministack/reset"

container_started=0
container_name="dataref-ministack-${USER:-user}-$$"
...
start_ministack_if_needed() {
    ...
    if [[ "$MANAGE_CONTAINER" == "auto" && "$DATAREF_MINISTACK_ENDPOINT" != "$DEFAULT_ENDPOINT" ]]; then
        return 0
    fi
