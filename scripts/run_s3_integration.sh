...
MINISTACK_IMAGE="${REFLAKE_MINISTACK_IMAGE:-nahuelnucera/ministack}"
MINISTACK_HOST="${REFLAKE_MINISTACK_HOST:-127.0.0.1}"
MINISTACK_PORT="${REFLAKE_MINISTACK_PORT:-4566}"
DEFAULT_ENDPOINT="http://${MINISTACK_HOST}:${MINISTACK_PORT}"
MANAGE_CONTAINER="${REFLAKE_MINISTACK_MANAGE_CONTAINER:-auto}"

export REFLAKE_MINISTACK_ENDPOINT="${REFLAKE_MINISTACK_ENDPOINT:-$DEFAULT_ENDPOINT}"
export REFLAKE_MINISTACK_ACCESS_KEY="${REFLAKE_MINISTACK_ACCESS_KEY:-test}"
export REFLAKE_MINISTACK_SECRET_KEY="${REFLAKE_MINISTACK_SECRET_KEY:-test}"
export REFLAKE_MINISTACK_REGION="${REFLAKE_MINISTACK_REGION:-us-east-1}"

HEALTH_URL="${REFLAKE_MINISTACK_ENDPOINT%/}/_ministack/health"
RESET_URL="${REFLAKE_MINISTACK_ENDPOINT%/}/_ministack/reset"

container_started=0
container_name="reflake-ministack-${USER:-user}-$$"
...
start_ministack_if_needed() {
    ...
    if [[ "$MANAGE_CONTAINER" == "auto" && "$REFLAKE_MINISTACK_ENDPOINT" != "$DEFAULT_ENDPOINT" ]]; then
        return 0
    fi
