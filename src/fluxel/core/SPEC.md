...
| `flx checkout <branch>` | O(1) | `cli.py` → `repository.checkout()` → `client_state.write_head()` | Switches current branch (rewrites HEAD file atomically). No data movement. |
| `flx restore <ref>` | O(M) — M = manifest entries | `cli.py` → `repository.restore_files()` → manifest reader + blob reads | Restores files from a branch or commit ref into the working tree. Supports `--path` filters (repeatable) and `--force` to overwrite existing files. |
